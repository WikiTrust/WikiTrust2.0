...
class create_entry:
    ...
    def create_environment(self, environment_name=''):
        try:
            ...
            self.db.rollback()
            return self.db(
                self.db.environment.environment_name == environment_name
            ).select(self.db.environment.id).first()
    ...
    def create_page(
        ...
        try:
            ...
            self.db.rollback()
            return self.db(self.db.page.page_id == page_id).select(
                self.db.page.id
            ).first()

    @autocommit
    def create_user(self, user_id=-1, user_name=''):
        try:
            ret = self.db.user.insert(user_id=user_id, user_name=user_name)
            return ret
        except:
            self.db.rollback()
            return self.db(self.db.user.user_id == user_id).select(
                self.db.user.id
            ).first()
    ...
    def create_revision(
        ...
        try:
            ...
            self.db.rollback()
            return self.db(self.db.revision.rev_id == rev_id).select(
                self.db.revision.id
            ).first()

    @autocommit
    def create_revision_log(
        self, version='', stage='', page_id=-1, last_rev=None, lock_date=None
    ):
        x = self.db.revision_log.version == version
        y = self.db.revision_log.page_id == page_id
        q = self.db(x & y).select().first()
        if (q == None):
            ret = self.db.revision_log.insert(
                version=version,
                stage=stage,
                page_id=page_id,
                last_rev=last_rev,
    ...
    def create_user_reputation(
        self, version='', user_id=-1, environment=None, reputation_value=0
    ):
        ...
        if (q == None):
            ret = self.db.user_reputation.insert(
                version=version,
                user_id=user_id,
    ...
    def create_text_storage(
        self, version='', page_id=-1, rev_id=-1, text_type='', blob=''
    ):
        ret = self.db.text_storage.insert(
            version=version,
            page_id=page_id,
            rev_id=rev_id,
            text_type=text_type,
    ...
    def create_triangles(
        ...
        ret = self.db.triangles.insert(
            version=version,
            page_id=page_id,
            rev_id_1=rev_id_1,
            rev_id_2=rev_id_2,
            rev_id_3=rev_id_3,
            reputation_inc=reputation_inc
        )
        return ret

    @autocommit
    def create_text_diff(self, version='', rev_id_1=-1, rev_id_2=-1, info=''):
        ret = self.db.text_diff.insert(
            version=version, rev_id_1=rev_id_1, rev_id_2=rev_id_2, info=info
        )
        return ret

    @autocommit
    def create_text_distance(
        self, version='', rev_id_1=-1, rev_id_2=-1, distance=0.0
    ):
        ...
        q = self.db(x & y & z).select().first()
        if (q == None):
            ret = self.db.text_distance.insert(
                version=version,
                rev_id_1=rev_id_1,
                rev_id_2=rev_id_2,
                distance=distance
