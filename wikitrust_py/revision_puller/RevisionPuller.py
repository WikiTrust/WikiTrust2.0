...
def get_all_revisions(
    ...
    end_time = pywikibot.Timestamp.fromtimestamp(
        datetime.now(tz=timezone.utc).timestamp()
    )
    return list(
        page.revisions(
            reverse=not recent_to_oldest,
...
def get_revisions_between(
    page: pywikibot.page.Page,
    revision1: pywikibot.page.Revision,
    revision2: pywikibot.page.Revision,
    recent_to_oldest=True
):
    """
    ...
    if (recent_to_oldest and firstRevTime <= secondRevTime):
        raise ValueError(
            "Second revision must occur before first revision if sorting by recent to oldest"
        )
    elif (not recent_to_oldest and firstRevTime >= secondRevTime):
        raise ValueError(
            "First revision must occur before second revision if sorting by oldest to recent"
        )
    return get_latest_revisions(
        page, recent_to_oldest, start_time=firstRevTime, end_time=secondRevTime
    )
