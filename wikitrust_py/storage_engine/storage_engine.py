...
from wikitrust_py import consts

from wikitrust_py.computation_engine.wikitrust_algorithms.author_reputation import version

__all__ = ['StorageEngine']
__num_revs_per_slot__ = 10  # DO NOT CHANGE THIS VALUE LIGHTLY, if the slot num changes, all previously saved pages/revisions may have the wrong slot calculated and will not load.
...
class StorageEngine(object):
    ...
    def _gcs_write(self, bucketname, filename, content, type='text/plain'):
        ...
        :return: Nothing.
        """
        if (consts.__USE_GCS__ == True):
            bucket = self.GCS.get_bucket(bucketname)
            blob = storage.Blob(filename, bucket)
            blob.upload_from_string(content, content_type=type)
        else:  # uses a local folder to store the data with a similar structure to GCS if we aren't using GCS for storage (used for debugging)
            os.makedirs(
                "./local_gcs/" + bucketname + "/" + filename.split("/")[0],
                exist_ok=True
            )
            with open("./local_gcs/" + bucketname + "/" + filename, "wb") as f:
                f.write(content)
    ...
    def _gcs_read(self, bucketname, filename):
        ...
        if (consts.__USE_GCS__ == True):
            ...
            local_file_path = "./local_gcs/" + bucketname + "/" + filename
