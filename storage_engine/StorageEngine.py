...
class StorageEngine(object):
    ...
    def store(self, revision):
        ...
        with self.lock:
            ...
            self.revision_list.append(revision)
            if len(self.revision_list) >= self.num_revs_per_file:
                self.__write_revisions()
            return True

    def __write_revisions(self):
        """Compresses and stores the revisions in self.revision_list in gcs"""
        if len(self.revision_list) > 0:
            revisions_json = bytes(json.dumps(self.revision_list), 'utf-8')
    ...
    def read_revision(self, revision_id=None, path=None, do_cache=True):
        ...
        if s is not None:
            revision_list = json.loads(zlib.decompress(s).decode('utf-8'))
            try:
                return revision_list[revision_id]
            except:
                return None
        else:
            return None
...
if __name__ == '__main__':
    se = StorageEngine(bucket_name='gcs-testing', num_revs_per_file=2)
    ...
    for i in range(4):
        print("Reading revision " + str(i))
        print(se.read_revision(i))
        print("===================")
