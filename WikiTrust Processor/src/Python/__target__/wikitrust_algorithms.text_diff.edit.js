...
var __name__ = 'wikitrust_algorithms.text_diff.edit';
export var Edit =  __class__ ('Edit', [object], {
	__module__: __name__,
	MOVE: 0,
	INSERT: 1,
	DELETE: 2,
	get __init__ () {return __get__ (this, function (self, edit_type, origin, destination, length) {
	...
	get __str__ () {return __get__ (this, function (self) {
		if (self.edit_type == self.MOVE) {
			return __mod__ ('MOVE   : %d -> %d, L= %d', tuple ([self.origin, self.destination, self.length]));
		}
		if (self.edit_type == self.DELETE) {
			return __mod__ ('DELETE : %d, L: %d', tuple ([self.origin, self.length]));
		}
		if (self.edit_type == self.INSERT) {
			return __mod__ ('INSERT : %d, L: %d', tuple ([self.destination, self.length]));
		}
...
//# sourceMappingURL=wikitrust_algorithms.text_diff.edit.map