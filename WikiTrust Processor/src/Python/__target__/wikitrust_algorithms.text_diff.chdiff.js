...
var __name__ = 'wikitrust_algorithms.text_diff.chdiff';
export var MOVE = 0;
export var INSERT = 1;
export var DELETE = 2;
...
export var print_edit_diff = function (diff_cmds) {
	for (var [cmd, i1, i2, l] of diff_cmds) {
		if (cmd == MOVE) {
			print (__mod__ ('MOVE   : %d -> %d, L= %d', tuple ([i1, i2, l])));
		}
		if (cmd == DELETE) {
			print (__mod__ ('DELETE : %d, L: %d', tuple ([i1, l])));
		}
		if (cmd == INSERT) {
			print (__mod__ ('INSERT : %d, L: %d', tuple ([i1, l])));
		}
	}
};
...
//# sourceMappingURL=wikitrust_algorithms.text_diff.chdiff.map