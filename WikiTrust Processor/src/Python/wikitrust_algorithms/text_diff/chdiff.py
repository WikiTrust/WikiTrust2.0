...
import heapq
# --- Executed only by CPython (Not Transcrypt) because unittest is unsupported ---
import unittest # __:skip
...
max_matches = 30
# This is the maximum number of deleted chunks that we keep.
max_del_chunks = 80
# Minimum length of a deleted chunk
min_del_chunk_len = 8
...
def fast_compute_edit_list(s1, s2):
    ...
    intermediate = edit_diff_tichy(s1[j: l1 - k], s2[j: l2 - k], idx)

    new_intermediate = [(m, i1 + j, i2 + j, l) for (m, i1, i2, l) in intermediate]
    return initial_move, new_intermediate, final_move
...
def edit_diff_greedy(words1, words2, index2):
    ...
    prev_matches = []
    for i1 in range(len1 - greedy_hash_words + 1):
...
def edit_diff_tichy (w1, w2, idx2):
    ...
    while i2 < len2 - 1:
        max_quality = None
        ws = (words2 [i2], words2 [i2 + 1])
        if ws in index1:
            for i1 in index1 [ws]:
                # Now that we found a partial match, determine the length
                if not (matched1 [i1] or matched1 [i1 + 1]):
                    l = 2
                    while (i1 + l < len1 and
                           i2 + l < len2 and
